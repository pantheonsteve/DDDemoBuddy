...
class AuthUI {
      ...
      <div class="auth-container">
        ...
        <div class="auth-body">
          <div class="account-info">
            ...
            <div class="account-status ${isPro ? 'pro' : 'free'}">
              ${isPro ? 'Pro' : 'Free Plan'}
            </div>
          </div>
          ...
          ${isPro ? `
            ...
            <button class="auth-action-btn" data-action="syncNow">
              🔄 Sync Now
            </button>
            
            <button class="auth-action-btn secondary" data-action="manageBilling">
              💳 Manage Subscription
            </button>
          ` : `
            <div class="upgrade-prompt">
              ...
              <div class="pricing">
                <button class="upgrade-btn monthly" data-action="upgrade" data-plan="monthly">
                  $10/month
                </button>
                <button class="upgrade-btn annual" data-action="upgrade" data-plan="annual">
                  $99/year <span class="save-badge">Save 17%</span>
                </button>
              </div>
            </div>
          `}
  ...
  async handleAction(e) {
    ...
    switch (action) {
      ...
      case 'manageBilling':
        const billingResult = await this.cloud.getBillingPortalUrl();
        if (billingResult.success) {
          window.open(billingResult.url, '_blank');
        } else {
          this.showError(billingResult.error);
        }
        break;
      ...
      case 'upgrade':
        try {
          const priceId = plan === 'annual' 
            ? DEMOBUDDY_CONFIG.STRIPE_PRICE_ANNUAL 
            : DEMOBUDDY_CONFIG.STRIPE_PRICE_MONTHLY;
          
          console.log('[AuthUI] Creating checkout for price:', priceId);
          
          if (!priceId) {
            this.showError('Price ID not configured. Check config.js');
            break;
          }
          
          const checkoutResult = await this.cloud.getCheckoutUrl(priceId);
          console.log('[AuthUI] Checkout result:', checkoutResult);
          
          if (checkoutResult.success && checkoutResult.url) {
            window.open(checkoutResult.url, '_blank');
          } else {
            this.showError(checkoutResult.error || 'Failed to create checkout session');
          }
        } catch (err) {
          console.error('[AuthUI] Upgrade error:', err);
          this.showError(err.message || 'Upgrade failed');
        }
        break;
    }
  }
